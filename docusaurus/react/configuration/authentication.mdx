...
You'll need to modify the `authorizedFetch` method, and acquire the correct scopes for the URL you're trying to call.
...
const swr = {
  fetcher: (path: string) =>
    authorizedFetch((import.meta.env.VITE_API_URL) + path).then(
