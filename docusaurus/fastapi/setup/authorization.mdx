...
should head over to the [official multi-tenant documentation](https://intility.github.io/fastapi-azure-auth/multi-tenant/azure_setup).
...
Copy the `Application (Client) ID` and `Directory (tenant) ID`, we'll need these for later. Copy them into your `.env` file:
