...
Copy the token and [add it to GitLab CI/CD variables](/configuration/gitlab#adding-cicd-variables) with the key `SONAR_TOKEN`.
...
Select `Quality Gate Status`, and `Image URL only`. You can then use the Image URL and project dashboard URL to [create a badge in GitLab](/configuration/gitlab#adding-badges).
