...
Create a project, the name should be the same as the GitLab project slug.  
If you use something other than the project slug, you'll need to modify the `OPENSHIFT_PROJECT` variable in the top of the .gitlab-ci.yml file.  
Add a fitting display name and description if you feel like it.

### CLI

The name should be the same as the GitLab project slug.  
If you use something other than the project slug, you'll need to modify the `OPENSHIFT_PROJECT` variable in the top of the .gitlab-ci.yml file.
```bash
...
```bash
oc policy add-role-to-user admin -z gitlab-builder
```
