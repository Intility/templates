...
But writing an OpenAPI documentation is a tedious task, both to write the first time as well as maintaining and updating.
So in this template we'll be using the [express-jsdoc-swagger](https://brikev.github.io/express-jsdoc-swagger-docs/#/) for autogenerating the OpenAPI documentation as well as hosting the Swagger page on our API.

***NOTE: There is currently an issue with the Authorization flow built into Swagger doc. There fore we will be hosting our own redirect logic. This can be seen in the `src/static` folder**
...
But this might break the default redirection logic provided by us.
