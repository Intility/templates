...
Remember to set up the [GitLab Deploy Token](/cra/configuration/gitlab#add-gitlab-deploy-token)!
:::
...
Copy the `token` field at the bottom of the page, and [add it to GitLab CI/CD variables](/cra/configuration/gitlab#adding-cicd-variables) with the key `OPENSHIFT_TOKEN`.
...
Copy the contents from the `token` section and [add it to GitLab CI/CD variables](/cra/configuration/gitlab#adding-cicd-variables) with the key `OPENSHIFT_TOKEN`.
