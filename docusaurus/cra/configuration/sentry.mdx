...
In the code example under Connecting the SDK to Sentry, copy the `dsn` value, and [add it to GitLab CI/CD variables](/cra/configuration/gitlab#adding-cicd-variables) with the key `SENTRY_DSN`.
...
Create it, copy the token and [add it to GitLab CI/CD variables](/cra/configuration/gitlab#adding-cicd-variables) with the key `SENTRY_AUTH_TOKEN`.
