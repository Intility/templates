...
Runs once the `build` job has finished, and only in the `master` or `main` branch.

### `deploy:dev`

Deploys the application to OpenShift using the Helm chart.
Runs once the `image:dev` and `test` jobs have succeeded, and only in the `master` or `main` branch.
